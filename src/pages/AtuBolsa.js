...
import { Select, Input, Text, Checkbox, CheckboxGroup, Wrap, Box, Button, Flex, useToast, Modal, ModalOverlay, ModalContent, ModalHeader, ModalFooter, ModalBody, ModalCloseButton, useDisclosure } from '@chakra-ui/react';
...
const AtuBolsa = () => {
  ...
  const [notNf, setNotNf] = useState('');
  const [inNf, setInNf] = useState('');
...
`;
  ...
  const handleNotNfChange = (event) => {
    const value = event.target.value;
    if (option === 'Apenas') {
      setInNf(value);
      setNotNf('');
    } else if (option === 'Exceto') {
      setNotNf(value);
      setInNf('');
    } else {
      setNotNf('');
  ...
  const fetchDatanf = async () => {
    ...
    try {
      ...
      const paramsNf = {
        ...
        notNf: notNf // Adicionando o valor do input como notNf
      };
      ...
      case 'prest':
        return (
          ...
          <table style={{ marginTop: '10px', width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {getSummaryPrest().map((item, index) => (
                <tr key={index}>
    ...
    <Box p={2}>
      ...
      >
        <Flex direction="column" ml="5px" mt="2px" maxHeight="100vh">
          ...
          <Flex mr="10px">
            <Flex direction="column" h="90%" w="70%">
              <Flex align="center" mb="10px">
                <Text w="28%" mr="3px" color="white" fontSize="18px">
                  Data inicial:
                </Text>
                <DatePicker
              ...
              <Flex align="center" direction="row">
                <Select w="28%"  bg="gray.100" placeholder='Todas' onChange={handleOptionChange}>
                ...
                <Input
                  ...
                  w="63%"
                  ml="5px"
                  placeholder="NF - Separar por vírgula"
                  value={option === 'Apenas' ? inNf : notNf}
                  onChange={handleNotNfChange}
                  isDisabled={option === ''}
                />
            ...
            >
              <CheckboxGroup
                ...
                w="100%"
              >
                <Wrap direction="column" spacing="10px" w="100%">
      ...
      <Modal isOpen={isOpen} onClose={onClose} size="2xl">
        ...
        <ModalContent>
          ...
          <ModalBody maxHeight="70vh" overflowY="auto">
            <Button mr={2} onClick={() => handleViewChange('prest')}>Por Prestador</Button>
