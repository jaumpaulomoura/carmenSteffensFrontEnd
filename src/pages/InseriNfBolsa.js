...
export default function InseriNfBolsa() {
    ...
    try {
	  ...
	  const jsonData = XLSX.utils.sheet_to_json(sheet, { 
        ...
        const dataWithIds = jsonData.map((rowData) => {
          ...
          const dia = dataEmissao.getDate().toString().padStart(2, '0');
  ...
  const enviarJsonParaApi = async () => {
    const api_url = "http://192.168.12.58:5000/api/inseriNfBolsa"; // Substitua pela URL correta da sua API
    const jsonDataToSend = dataWithIds; // Use os dados que você deseja enviar para a API
