...
export default function FichaExpedicao() {
    ...
    reader.onload = async (e) => {
      try {
        ...
        jsonData.forEach((item) => {
          if (item.ano !== undefined && item.ano !== null && /^[0-9]+$/.test(item.ano) && !/^0+/.test(item.ano)) {
            const ano = item.ano.toString();
            if (!fichasByYear[ano]) {
              fichasByYear[ano] = [];
            }
            fichasByYear[ano].push(item.ficha);
          }
        });
  
        for (const ano in fichasByYear) {
          const batches = chunkArray(fichasByYear[ano], 999);
          for (let i = 0; i < batches.length; i++) {
            const batchFichas = batches[i]; 
           
            await fetchResultsFromBackend(ano, batchFichas);
  
            if (i < batches.length) {
             
              await new Promise((resolve) => setTimeout(resolve, 30000));
              console.log(ano, batchFichas)
  ...
  const fetchResultsFromBackend = async (ano, batchFichas) => { 
    try {
      const api_url = "http://192.168.12.58:5000/api/queryFicha";
      
      const params = {
        ano: ano,
        ficha: batchFichas.join(","), 
      };
      console.log(params)
  ...
  const handleSubmit = async (e) => {
    ...
    if (valorCompleto) {
      if (valorCompleto.length === 10) {
        ...
        const fullValue = valorCompleto.replace(/^0+/, ''); 
